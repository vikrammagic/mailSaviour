...
function validate() {
 ...
 try {
   ...
   const bodyElement = iframeDocument.getElementById('editedText');
   ...
   if (bodyElement) {
     ...
     console.error("Element with id='editedText' not found inside iframe.");
   }
 } catch (error) {
  ...
  const wordCount = bodyHTML.split(/\s+/).length;
    ...
    default:
      ...
      fetch('/sendGmail', {
        ...
        body: JSON.stringify({
          ...
          isHTML: document.getElementById('editor').checked
        })
      })
